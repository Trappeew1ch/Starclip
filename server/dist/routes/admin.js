...
router.get('/stats', async (req, res) => {
    try {
        const [totalUsers, activeOffers, pendingClips, totalClips, totalPaidOut, pendingPayouts] = await Promise.all([
            prisma.user.count(),
...
router.post('/clips/:id/approve', async (req, res) => {
    try {
        ...
        if (clip.user.referredById) {
            ...
            if (referralBonus > 0) {
                prismaTx.push(prisma.user.update({
                    where: { id: clip.user.referredById },
                    data: { balance: { increment: referralBonus } }
                }));
                prismaTx.push(prisma.transaction.create({
                    data: {
...
router.post('/offers', async (req, res) => {
    try {
        const { name, title, type, imageUrl, avatarUrl, bannerUrl, totalBudget, cpmRate, language, platforms, description, requirements, assetsLink, daysLeft } = req.body;
